...
- `Script + Interval` ((#script-check))- These checks depend on invoking an external application
...
- `TCP + Interval` ((#tcp-check))- These checks make a TCP connection attempt to the specified
  IP/hostname and port in the health check defintion.

- `UDP + Interval` ((#udp-check))- These checks direct the client to periodically send UDP datagrams
  to the specified IP/hostname and port in the health check definition.
...
- `Time to Live (TTL)` ((#time-to-live-(ttl)-check))- These checks retain their last known state
  for a given TTL.
  
- `Docker + Interval` ((#docker-check))- These checks depend on invoking an external application which
...
- `H2ping + Interval` ((#h2ping-check))- These checks test an endpoint that uses http2
...
The state of the check must be updated periodically over the HTTP interface.
...
In Consul 0.9.0 and later, Docker checks are not enabled by default.
...
add the service identifier after the `gRPC` check's endpoint in the following format `/:service_identifier`.
